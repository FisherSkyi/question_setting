...
function get_head(n, lst) {
    return n === 1 ? null : pair(head(lst), get_head(n-1, tail(lst)));
}
...
//     return list_ref(accessor, head(num_list)-1) % 2 === 1 
//     ? pair("insert", step_choice(tail(num_list), 
//     append(list(get_head(head(num_list), accessor)), 
//     pair(list_ref(head(num_list)-1) + 1, get_tail(head(num_list), accessor))))) 
//     : pair("remove", step_choice(tail(num_list), 
//     append(list(get_head(head(num_list), accessor)), 
//     pair(list_ref(head(num_list)-1) + 1, get_tail(head(num_list), accessor)))));
// }

// step_choice(num_list2, build_list(x => 0, length(rings)));


function steps(num_list) {
    return is_null(num_list) ? null
    : pair(list("step",head(num_list)), steps(tail(num_list)));
}

display_list(reverse(steps(num_list2)));