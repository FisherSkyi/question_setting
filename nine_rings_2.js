...
function steps_to_free_configuration(desired_first_state, my_ring) {
    const rings = tail(my_ring);
    function generate_num_list(n) {
        return n === 2 ? list(1, 2, 1) : append(generate_num_list(n - 1), pair(n, generate_num_list(n - 1)));
    }
    const len = length(rings);
    ...
    return reverse(steps(num_list2, step_list2));
}

const rings = list(list(pair("on", 4)), list(pair("off", 3)), list(pair("on", 2)), list(pair("off", 1)));
    if (desired_first_state === "on") {
        return steps_to_free_configuration("on", rings);
    } else {
        
    }
steps_to_free_configuration("on", rings);